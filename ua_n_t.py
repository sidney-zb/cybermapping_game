...
def ADpayoff_left(x, c, n, pe, round):
    # 博弈初始配置
    # 端口开放
    vh = 4
    # 端口关闭
    ...
    if round == n:
        ...
        AD_l = (
            ...
            * (
                xt * (vh - (round + 1) * c)
                + (1 - xt) * (ADpayoff_left(x, c, n, pe * (1 - xt), round + 1))
            )
            + (1 - pe) * vh
        )
        return AD_l
...
def ADpayoff_right(x, c, n, pe, round):
    # 博弈初始配置
    # 端口开放
    vh = 4
    # 端口关闭
    ...
    if round == n:
        ...
        AD_r = pe * 1 * (ADpayoff_right(x, c, n, pe * (1 - xt), round + 1)) + (
            1 - pe
        ) * (vl - round * c)
        return AD_r
    else:
...
# 创建变量范围
t_values = np.linspace(0, 1, 100)  # 变量 xt 的范围
...
ax.set_title("ADpayoff as a 3D Function of t and n")
